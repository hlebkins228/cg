...
def draw_edges(canvas, edges):      # used
...
def make_edges_list(figures):       # used
    edges = list()
    for fig in figures:
...
def find_extrimum_Y_figures(figures):       # used
    yMin = figures[0][0].y
    yMax = figures[0][0].y
    for fig in figures:
        for p in fig:
...
def make_link_list(Ymin=0, Ymax=CANVAS_WIDTH):      # used
    link_list = dict()
...
def update_y_group(y_groups, x_start, y_start, x_end, y_end):       # used
        if y_start > y_end:
...
def iterator_active_edges(active_edges):        # used
    i = 0
...
def add_active_edges(y_groups, active_edges, y):        # used
    if y in y_groups:
...
def draw_act(canvas, active_edges, y, colour):      # used
...
def CAP_algorithm_with_ordered_list_of_edges(canvas, polygon, colour="black", delay=False):      # used
